...
# 'Hudson, Kate' >>> 'K. Hudson'
# apellido = nombre_completo[:6] Incorrecto !!!! No debo usar constantes como el número 6
...
# 'Torres, Ana'
for nombre_completo in nombres_completos:
    _, nombre = nombre_completo.split(', ') # ['Torres', 'Ana']
...
for i in range(len(departamentos)):
    if departamentos[i] == 'Logística':
        ...
        fecha = fechas_nacimiento[i] # '15/05/1943'
        # dn = int(fecha[:2])
        # mn = int(fecha[3:5])
        # an = int(fecha[6:])
        dn, mn, an = [int(n) for n in fecha.split('/')] # para tirar una magia
        edad = ah - an
