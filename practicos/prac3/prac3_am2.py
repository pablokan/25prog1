...
for i in range(len(lista_nacidos2008)):
    if lista_nacidos2008[i] == 'f':
        ...
        cantidad = int(lista_nacidos2008[i-1])
        nombre = lista_nacidos2008[i-2]
        datos_08m.append([cantidad, nombre])
...
print('1) La diferencia en cantidad de bebés que existe entre los nombres de misma posición y mismo sexo. Se solicita posición y sexo. (Ver salidas de ejemplo).')
...
if sexo == 'm':
    ...
    print(f'Nenas en posición {pos+1}: ')
...
nombres_08f = [dato[1] for dato in datos_08f]
nombres_08m = [dato[1] for dato in datos_08m]
